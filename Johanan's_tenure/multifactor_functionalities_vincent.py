...
def monte_carlo_simulation(n_simulations,mbetaA,mbetaB,mbetaC,type,inn,outt,placemonthly,spymonthly):
    results = []
    yearly_returns=[]
    ...
    for i in range(n_simulations):
        if type == 'stop':
            oos1_new_performance=new_run_with_backtest_rebalance_stop(inn,outt,mbetaA,mbetaB,mbetaC)
        elif type == 'rebalance':
...
def calculate_volatility(weights_df):
    ...
    start_5_years = weights_df.index[0] - relativedelta(years=5)  
    end_5_years = weights_df.index[0] - relativedelta(months=1)
    ...
    for month in weights_df.index:
        ...
        end_5_years = weights_df.index[0] - relativedelta(months=1-n)

        # re run prices data for new window (+1 month each iteration)
        volatility_prices = extract_stock_data(price_monthly_data, volatility_rebal.columns.tolist(), start_5_years, end_5_years)
